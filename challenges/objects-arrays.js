...
const universities = graduates.map(a => a.university);
universities.sort();
...
const unisWithUni = graduates.filter(a => a.university.includes("Uni"));
...
const displayNames = zooAnimals.forEach((element) => {
  return `Name: ${element.animal_name}, Scientific Name: ${element.scientific_name}`
});
...
const lowCaseAnimalNames = zooAnimals.map(a => a.animal_name);
